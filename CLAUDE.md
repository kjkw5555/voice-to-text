...
- **Local model persistence** (`ensure_model_exists`, `verify_model_hash`, `is_internet_available`):
  ...
  file already exists. See `plan/2026-05-08_local-model-persistence.md` for the original design.
...
- **Progress display** (`mode`: `none` / `bar` / `full`): controls both whisper's own verbose output
  and the tqdm progress bar; `detect_audio_language` runs an extra language-ID pass up front only when
  `mode` is `bar` or `full`, purely to print the detected language before transcription starts.
- **CLI entrypoint** (`if __name__ == "__main__":` block): when the input path is a directory, all
